...
impl App {
    ...
    fn filtered(&self) -> Vec<(usize, &Snippet)> {
        if self.search.is_empty() {
            self.snippets.iter().enumerate().collect()
        } else {
            let q = self.search.to_lowercase();
            self.snippets.iter().enumerate().filter(|(_, s)| {
...
fn draw_list(f: &mut ratatui::Frame, area: Rect, app: &App) {
    ...
    let rows: Vec<Row> = filtered.iter().enumerate().map(|(i, (_, s))| {
        ...
        Row::new(vec![
            ...
            Cell::from(s.language.clone()).style(Style::default().fg(snippet::language_color(&s.language))),
        ])
    }).collect();
    ...
    .block(
        Block::default()
            .borders(Borders::ALL)
            .border_style(Style::default().fg(if app.mode == Mode::Search { GREEN } else { DIM_GREEN }))
...
fn draw_preview(f: &mut ratatui::Frame, area: Rect, app: &App) {
    let block = Block::default()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(if app.mode == Mode::View { GREEN } else { DIM_GREEN }))
    ...
    if let Some(s) = app.selected() {
        ...
        let meta = Paragraph::new(vec![
            Line::from(vec![
                Span::styled(&s.title, Style::default().fg(GREEN).add_modifier(Modifier::BOLD)),
                Span::styled(format!("  [{}]", s.language), Style::default().fg(snippet::language_color(&s.language))),
            ]),
            Line::from(Span::styled(format!("tags: {}", tags_str), Style::default().fg(DIM_GREEN))),
        ]);
        f.render_widget(meta, chunks[0]);

        // Code
        let code_lines: Vec<Line> = s.code.lines().enumerate().skip(app.code_scroll).map(|(i, l)| {
            Line::from(vec![
        ...
        let empty = Paragraph::new(Span::styled("No snippet selected", Style::default().fg(DIM_GREEN)))
            .block(block);
